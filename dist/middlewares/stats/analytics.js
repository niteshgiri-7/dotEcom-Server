...
export const findGrowthRate = TryCatch(async (req, res, next) => {
    ...
    const getLatestTransactions = Order.find({})
        ...
        .limit(4).lean();
    const [thisMonthProducts, lastMonthProducts, thisMonthOrders, lastMonthOrders, thisMonthUsers, lastMonthUsers, productCount, userCount, //total number of users in the system
    allOrders, //total of each individual orders to calculate the total revenue generated
    lastSixMnthsOrders, allProducts, allUsersGender, latestTransactions,] = await Promise.all([
    ...
    const totalRevenue = allOrders
        .filter((order) => order.status === "delivered")
        .reduce((total, order) => (total += order.total), 0);
    const gender = allUsersGender.reduce((obj, user) => {
        if (user.gender === "male")
            obj.male++;
        else
            obj.female++;
    ...
    const stats = {
        ...
        overviewCount: [{
                name: "Product",
                rate: calculatePercentage(lastMonthProducts.length, thisMonthProducts.length),
                count: productCount,
            },
            {
                name: "User",
                rate: calculatePercentage(lastMonthUsers.length, thisMonthUsers.length),
                count: userCount
            },
            {
                name: "Orders",
                rate: calculatePercentage(lastMonthOrders.length, thisMonthOrders.length),
